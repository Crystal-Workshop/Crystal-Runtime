...
// Per-script runner
static void runScriptThread(const std::string archivePath, ArchiveFileEntry entry, DataModel* dm, InputState* inputState, GLFWwindow* wnd) {
...
LuaScriptManager::LuaScriptManager(const std::string& archivePath,
    ...
    GLFWwindow* wnd)
    : m_archivePath(archivePath), m_files(files), m_dataModel(dm), m_inputState(inputState), m_wnd(wnd), m_impl(new Impl())
{}
...
ArchiveFileEntry e = entry;
            ...
            GLFWwindow* wnd = m_wnd;
m_impl->threads.emplace_back([archiveCopy, e, dm, inputState, wnd]() {
                runScriptThread(archiveCopy, e, dm, inputState, wnd);
            });
}
    }
}
